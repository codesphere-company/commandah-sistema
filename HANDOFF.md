...
### Cobrança de fiado por WhatsApp com itens do dia + chave Pix (2026-09-24)

Pedido do Fabricio: a mensagem de cobrança de fiado enviada por WhatsApp (`whatsAppCollectionMessage`, disparada pelo botão "Cobrar via WhatsApp" e pelo prompt automático de `promptWhatsAppCollection` após fechar uma comanda com sobra em fiado) só citava o valor devedor total, sem detalhar o consumo nem informar como pagar.

- Nova função `todaysSalesForMember(memberId)` (`index.html:860`) filtra `state.sales` pelo sócio e por `closedAt`/`openedAt` de hoje (`todayStr()`).
- `whatsAppCollectionMessage(member)` agora monta um bloco "Consumo de hoje:" listando cada item não cancelado (`qty`x`nome` — subtotal) de todas as comandas fechadas hoje por aquele sócio, e um bloco com a chave Pix do estabelecimento (`state.settings.pixKey`, mesmo campo já usado no rodapé do cupom em Configurações do Sistema). Ambos os blocos são omitidos silenciosamente quando não há consumo hoje ou a chave Pix não está configurada — nunca aparece vazio/"undefined".
- Como as 3 chamadas de cobrança (bloqueio por débito, botão manual e prompt pós-fechamento) passam pelo mesmo `whatsAppCollectionMessage`, o ajuste vale pros três fluxos de uma vez.
- Testado por sintaxe JS + harness Node isolado (extração das funções reais via parsing de chaves balanceadas, não regex ingênuo) cobrindo 4 casos: consumo hoje + Pix configurada, sem consumo hoje + sem Pix, item cancelado ignorado, múltiplas comandas do mesmo dia agregadas sem misturar outro sócio — e validado por Fabricio no app real ("testei no localhost, deu certo").

### Cancelar item ou comanda inteira, com permissão por login (2026-09-24)

Pedido do Fabricio: "quando for cancelar, preciso que cancele o iten da comanda que eu escolher ou comanda interia. e isso quem pode é no login do usuário". Investigação prévia encontrou dois gaps: cancelamento de item individual já existia (`openCancelComandaItem`/`confirmCancelComandaItem`, no Histórico da comanda) mas sem nenhuma checagem de permissão, e não existia nenhum fluxo pra cancelar a comanda inteira de uma vez — só `deleteComandaOrder` (exclui pedido vazio/sem item enviado) e `cancelDelivery` (específico de delivery).

- Novo token de permissão `cancelar-comanda`, adicionado a `ROLE_PERMS.admin` e `ROLE_PERMS.caixa` (`index.html:807-809`) e registrado no grupo "Atendimento" da Matriz de Permissões (`openPermissionMatrix`, `index.html:2749`), então o dono pode revogar de um `caixa` específico sem mexer em `admin`. `cozinha` não ganhou o token (não faz sentido nesse papel).
- Botão "✕ Cancelar" do item, no Histórico da comanda (`renderComandaAuditHistory`, `index.html:4216`), agora só aparece com `hasPerm('cancelar-comanda')`; `openCancelComandaItem` ganhou `requirePerm('cancelar-comanda')` como defesa em profundidade (bloqueia mesmo se alguém chamar a função direto).
- Novo par de funções `openCancelComanda(saleId)`/`confirmCancelComanda()` (`index.html:4324`) pra cancelar a comanda inteira, acessível pelo botão "Cancelar Comanda" no menu "Mais Opções" da comanda (`index.html:3808`, ao lado de "Excluir Pedido", também gated por `hasPerm`). Modal pede confirmação com motivo opcional, no mesmo padrão visual do cancelamento de item. Bloqueia se a comanda estiver travada (`locked`) ou já fechada.
- `confirmCancelComanda` só devolve estoque (`restoreInsumosForItems`) dos itens que já tinham `sent=true` — itens ainda em rascunho (nunca enviados à cozinha) nunca tiveram estoque debitado, então não entram na devolução. Todos os itens ativos (enviados ou não) são marcados `canceled=true`, o que já zera `saleTotal` automaticamente. Cancela também os pedidos pendentes no Monitor de Cozinha (`OrdersStore.cancelBySale`) e fecha a comanda (`status='closed'` + flag auxiliar `s.canceled=true`/`canceledAt`/`canceledBy`/`canceledReason`), seguindo o mesmo padrão já usado por `cancelDelivery` (nunca inventa um novo valor de `status`, só a flag auxiliar) — assim os relatórios que já filtram por `status==='closed'` continuam funcionando, bastando adicionar `&&!s.canceled` em relatórios que não devem contar comanda cancelada como receita — **já feito, ver seção "Comanda cancelada não conta mais como receita" abaixo**.
- Testado por sintaxe JS + harness Node isolado com os trechos reais extraídos via parsing de chaves balanceadas (3 cenários: cancelamento completo com permissão — confirma que só o item `sent` entra na devolução de estoque, item em rascunho fica de fora; sem permissão — aborta antes de tocar em qualquer coisa; falha na devolução de estoque — aborta sem fechar a comanda). **Testado pelo Fabricio no localhost, confirmado que funcionou.**

### Carrinho no "Adicionar produto direto" da comanda (2026-09-24)

Pedido do Fabricio: "preciso poder adicionar mais de 1 iten por pedido". O campo de busca direta na comanda (`comandaQuickSearch`, feature de 2026-09-20 documentada acima) foi lançado como fluxo de **um produto por vez**: clicar num resultado abria um modal só daquele produto (quantidade + quem está pedindo), confirmar já enviava pra cozinha e fechava o modal — pra adicionar um segundo produto era preciso buscar de novo do zero. O "Novo Pedido" já suportava carrinho com vários produtos (`npCart`); esse ajuste leva o mesmo padrão pro atalho de busca direta.

- Globais trocados: `comandaQuickAddQty/comandaQuickAddProductId/comandaQuickAddCopos` (estado de um único produto) viraram `comandaQuickCart` (array, mesma forma de item que `npCart`).
- Clicar num resultado da busca agora chama `quickCartAdd(productId)` — adiciona direto no carrinho (incrementa se o produto já estiver lá) **sem abrir modal**, igual ao `npCartAdd` do Novo Pedido; o próprio botão do grid de busca ganhou o badge de quantidade (`inCart`) que o Novo Pedido já usava.
- Novo container inline `comandaQuickCartWrap` (`index.html:3821`, logo abaixo do grid de resultados da busca) renderizado por `renderComandaQuickCart()` — lista os itens acumulados com stepper de quantidade (`quickCartChangeQty`) e, pra produto de bar, stepper de copos (`quickCartChangeCopos`), mais o total e um botão único "Adicionar e enviar pra cozinha (N itens)".
- Esse botão abre `openComandaQuickAddConfirm()` — modal enxuto só pra escolher quem está pedindo (titular/dependente/avulso, mesma checagem de bloqueio por débito de sempre), reaproveitando a ideia do requester único do Novo Pedido (uma pessoa pro pedido inteiro, não por item). Confirmar chama `confirmComandaQuickAdd()`, reescrita pra iterar `comandaQuickCart` inteiro: um único `sendItemsToKitchen` pra todos os itens de uma vez, agrupamento por estação e impressão via `printNovoPedidoTickets` (mesmo helper multi-estação do Novo Pedido — antes o fluxo de um item só usava `printSingleTicket` direto).
- `comandaQuickCart` é zerado ao confirmar com sucesso e também toda vez que a comanda é aberta "do zero" (não em reaberturas internas do próprio fluxo — `openComanda(id, keepRequester)` só reresulta o carrinho quando `keepRequester` é falso, que é o caso de abrir a comanda a partir da lista; as reaberturas internas depois de cancelar o passo de "quem está pedindo" continuam com `keepRequester=true` e preservam o carrinho, pra não perder os itens já escolhidos).
- Testado por sintaxe JS + harness Node isolado com os trechos reais extraídos via parsing de chaves balanceadas: carrinho acumula produtos distintos com quantidades corretas, produto sem estoque é rejeitado com toast e não entra no carrinho, confirmação manda todos os itens numa única chamada de `sendItemsToKitchen`, impressão sai agrupada por estação (bar + cozinha testados juntos), total da comanda bate, carrinho zera depois de confirmar. **Testado pelo Fabricio no app real, confirmado ("blza deu certo").**

### Histórico da comanda agrupado por pedido, com "pedir novamente" do lote inteiro (2026-09-24)

Pedido do Fabricio (depois de confirmar o carrinho acima): "eu preciso que no historico dentro da comanda aberta. cada iten seja agrupado pelo pedido. exemplo: pedi 3 coisas, depois pedi mais 2. as 3 coisas agrupadas no pedido delas, porem com uma seta para que eu possa abrir ele e ver o que foi pedido, com a opção de pedir novamente." Antes disso, `renderComandaAuditHistory` (Histórico da comanda) era uma tabela achatada, uma linha por item, sem nenhuma noção de "isso tudo foi um único pedido".

- Mapeamento prévio: existem pelo menos 5 pontos que empurram item pra dentro de uma comanda (Novo Pedido, carrinho de busca direta, "pedir novamente" de item avulso, digitação clássica de produto na comanda — `commitDraftItem` — e o fluxo do totem/cardápio digital), mas **todos** passam por um único ponto de estrangulamento antes de virar pedido de cozinha: `sendItemsToKitchen(saleId, items, who, source, local)` (`index.html:2938`). Em vez de mexer nos 5+ pontos de origem, o "pedido" (lote) passou a ser marcado ali, no único lugar por onde tudo já converge — mesmo espírito de `saleTotal` já ser o único lugar que soma total de comanda.
- `sendItemsToKitchen` agora gera `const batchId = uid()` logo no início e carimba `item.batchId = batchId` em todo item do array recebido que ainda não tiver um (`if(!i.batchId)`) — isso preserva um `batchId` já carimbado de propósito (usado pelo "pedir novamente do lote", ver abaixo) em vez de sobrescrever. Como os itens são passados por referência e só entram em `s.items` depois que a função retorna sucesso, o carimbo já sai correto nos objetos que a tela salva.
- `renderComandaAuditHistory` (`index.html:4200`) agora agrupa `currentSaleDraft.items` por `batchId` (fallback `'legacy-'+idx` pra item antigo sem o campo, tratado como lote próprio de 1 item — nunca existiu carrinho antes desta sessão, então todo item pré-existente vira "lote de 1"). Lote com **só 1 item continua renderizando a linha de sempre**, sem cabeçalho nem seta — só lote com 2+ itens (Novo Pedido ou carrinho de busca com mais de um produto) ganha uma linha de cabeçalho colapsável ("▸ Pedido N", numerado só entre os lotes múltiplos, nessa ordem cronológica), com quantidade/valor somados (só dos itens não cancelados), solicitante (ou "Vários" se o lote tiver mais de um nome), situação agregada (Novo/Enviado/Cancelado/Parcial) e um botão "🔁 Pedir novamente (tudo)". Clicar no cabeçalho expande/recolhe (`toggleComandaAuditGroup`, estado em `comandaAuditExpanded`, um `Set` de `batchId`s abertos) e mostra as linhas originais dos itens do lote — **sem tocar no índice real** (`idx`) que os botões de cada item (`changeDraftQty`, `openReorderComandaItem`, `openCancelComandaItem` etc.) já dependem.
- Função nova `openReorderComandaBatch(batchKey)` (`index.html:4535`, ao lado do carrinho de busca direta): reaproveita a infraestrutura de carrinho recém-criada em vez de inventar um fluxo próprio — filtra os itens ativos (não cancelados) do lote, busca o preço/disponibilidade **atuais** do catálogo pra cada produto (mesmo padrão do "pedir novamente" de item avulso, que já não usa o preço congelado do histórico), soma quantidade quando dois itens do lote são o mesmo produto, avisa com toast e ignora silenciosamente produto descontinuado, e por fim popula `comandaQuickCart` e abre o mesmo modal `openComandaQuickAddConfirm()` do carrinho — ou seja, "pedir novamente" do lote inteiro vira um novo pedido, com seu próprio `batchId` novo, exatamente como pedir de novo manualmente.
- `comandaAuditExpanded` é zerado no mesmo ponto que `comandaQuickCart` dentro de `openComanda(id, keepRequester)` — só quando `keepRequester` é falso (abrir a comanda "do zero" pela lista), preservando o estado de expandido/recolhido nas reaberturas internas do próprio fluxo (ex. depois de confirmar um "pedir novamente").
- Testado por sintaxe JS + harness Node isolado com os trechos reais extraídos via parsing de chaves balanceadas (13 asserções): todo item de uma mesma chamada de `sendItemsToKitchen` recebe o mesmo `batchId`, chamadas diferentes geram `batchId` diferente, `batchId` pré-carimbado não é sobrescrito; lote de 2+ itens renderiza cabeçalho colapsável e lote de 1 item renderiza linha solta (sem "Pedido N"); expandir o cabeçalho mostra as linhas dos itens; `openReorderComandaBatch` monta o carrinho certo a partir de um lote (soma quantidade de produto repetido, ignora item cancelado do lote e produto descontinuado com toast de aviso, e abre o modal de confirmação uma única vez). **Testado pelo Fabricio no localhost, confirmado que funcionou.**

### Comanda cancelada não conta mais como receita nos relatórios (2026-09-24)

Dívida técnica aberta pela feature de cancelamento de comanda inteira (seção acima) e cobrada pelo Fabricio no mesmo dia: como comanda cancelada fecha com `status='closed'` + flag `s.canceled=true`, todo relatório que filtrava só por `status==='closed'` a contava como faturamento.

- A condição `s.status==='closed' && s.deliveryStatus!=='cancelado'` estava **duplicada literalmente em 7 lugares** (e em mais um site sem o guard de delivery), então em vez de espalhar `&& !s.canceled` por todos eles, criei um predicado único `saleCountsAsRevenue(s)` (`index.html:2932`, logo abaixo de `saleTotal`) que concentra as três condições: fechada, não cancelada como delivery, não cancelada como comanda. Mesmo padrão de ponto único de `saleTotal` (soma de comanda) e `sendItemsToKitchen` (carimbo de lote) — relatório novo que usar o predicado não pode mais esquecer o filtro.
...
### Correções rápidas da auditoria (2026-09-25)

Três itens da auditoria de 2026-09-12, reconferidos no código e ainda abertos. Resumo da auditoria no doc "Commandah — Estado atual e auditoria" (https://claude.ai/code/artifact/22b1e25b-f98a-4f8a-b8ab-38e7b0d466c4).

- **Aviso de sincronização parada**: o poll de 6s (`startApp`) tinha `catch(e){}` vazio. Se a rede caía, o Monitor de Cozinha e as comandas congelavam sem nenhum aviso. Agora a busca (`OrdersStore.load` + `storageApi.get(sales)`) fica num `try` separado da renderização e chama `noteSyncResult()`. Depois de 3 falhas seguidas (~18s, `SYNC_FAIL_THRESHOLD`), aparece um banner "Este aparelho parou de sincronizar", que reusa o `#localModeRoot` e empilha com os outros avisos. Ele some sozinho na primeira sincronização que der certo. Um erro de renderização não conta como falha de rede.
- **"Quem levou" na Matriz de Permissões**: entrou no grupo "Áreas e dispositivos". Antes, salvar a matriz tirava o acesso do caixa em silêncio. Quem já salvou uma customização antes disso precisa marcar a caixa de novo.
...
Fecha os dois achados da auditoria de 2026-09-12 (linhas "Grant residual de EXECUTE pra anon" e "`waToken`/`smsToken` no settings").

- **Migration `20260925000000_revoke_anon_e_tokens_settings.sql`**: as migrations antigas faziam `revoke ... from public`, mas o Supabase dá EXECUTE direto pro `anon` em toda função nova, e isso não sai com o `from public`. Agora `revoke execute ... from anon` em `next_order_number`, `consume_insumos`, `restore_insumos`, `close_sale`, `regenerate_print_agent_token` e nas funções de trigger `recalc_member_debt`/`audit_log_stamp`. Ficam com anon, de propósito: `print_agent_*` (o agente usa a chave anon + token) e as funções usadas dentro das policies (`current_tenant_id`, `current_staff_role`, `is_admin_like`, `app_data_key_class`).
- **Tokens de integração**: WhatsApp, SMS, canais digitais e backup em nuvem guardavam token em `cantina2:settings`, que todo operador lê. Nenhum era usado (não existe integração real). Os 4 campos saíram dos formulários, `stripIntegrationTokens()` tira da memória o que vier do banco no `loadAll`, e a migration apaga o que já estava salvo. Mesmo tratamento do secret do Pix na Fase 0.
...
- Testado: sintaxe JS; `stripIntegrationTokens` com os 4 grupos, `null` e `{}`; migration rodada duas vezes num Postgres em memória (PGlite) com os grants iguais à produção. Os tokens somem, o resto do settings e as outras chaves ficam iguais, anon perde EXECUTE e authenticated mantém. **Pendente aplicar em produção** (o MCP do Supabase estava fora do ar nesta sessão).

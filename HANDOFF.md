...
### Fase 1 — fundação, EM ANDAMENTO em 2026-08-31 (itens 1-2 concluídos; item 3 em progresso)
...
3. **`finalizeSale`/numeração de pedido/baixa de estoque atômicos via RPC — EM ANDAMENTO.** Investigação completa feita: a baixa de estoque (`consumeInsumosForItems`) tem 9 pontos de chamada (todos via `sendItemsToKitchen`), a numeração (`nextOrderNumber`) tem **11 pontos** (a maioria ao ABRIR comanda/delivery/agendamento, não só ao fechar venda — nenhuma das 11 funções chamadoras é `async` hoje, mas nenhuma depende de retorno síncrono, então converter é seguro), e o registro da venda em si só precisa mudar dentro de `finalizeSale`. Desenho fechado: 3 RPCs no Postgres com trava de linha (`FOR UPDATE`/`UPDATE...ON CONFLICT`) sobre os próprios blobs JSON em `app_data` — **sem precisar migrar `sales`/`insumos`/`orderCounter` pro relacional agora** (isso continua sendo a Fase 3, maior, separada):
   - `next_order_number(p_start)` — incremento atômico de `cantina2:orderCounter` via `INSERT...ON CONFLICT DO UPDATE...RETURNING` (atômico de forma nativa, sem SELECT separado).
   - `consume_insumos(p_consumptions jsonb)` — trava a linha de `cantina2:insumos`, confere se dá pra atender TUDO antes de descontar qualquer coisa (falha = rollback completo, nada é descontado pela metade), retorna `{insumoId,before,qty,after}` pro cliente montar o log de auditoria com números confiáveis.
   - `close_sale(p_sale_id, p_existing, p_sale_patch)` — grava a venda (insert novo ou merge num existente) com **idempotência real**: uma tabela nova `sale_close_receipts (tenant_id, sale_id, result)` faz com que um retry de rede (mesmo `sale_id`) devolva o resultado já gravado em vez de duplicar a venda.
   - Decisão importante: as 3 RPCs são **independentes** entre si (não uma transação gigante combinada) — cada uma atômica sozinha, chamadas em sequência de dentro de `finalizeSale`. Mais simples de implementar e revisar com segurança, e o único risco residual (falha exatamente ENTRE duas chamadas, ex. estoque descontado mas venda falhou ao gravar) é muito menor que o problema original.
   - **Status no fim desta sessão**: uma tentativa de implementação via agente `backend-senior` foi interrompida por rate limit da conta (`session limit`) logo no início, sem nenhum arquivo escrito (confirmado por `git status` limpo). O desenho completo acima (incluindo o SQL rascunho das 3 funções) está pronto pra retomar — só falta escrever a migration (`supabase/migrations/20260831020000_rpc_atomicas_venda.sql`, ainda não criada) e as mudanças em `index.html` (`consumeInsumosForItems` ~linha 2566, `nextOrderNumber` ~linha 5067 + seus 11 call sites, `finalizeSale` ~linha 3046). Nada disso foi commitado ainda.

## Pendências (próximos passos, backlog priorizado pelo scrum-master em 2026-08-31)

**Fase 1 — fundação (retomar):**
1. ~~Versionar `supabase/migrations/`~~ — feito (ver acima).
2. ~~Limpar PII órfã~~ — feito (ver acima).
3. `finalizeSale`/numeração de pedido/baixa de estoque atômicos via RPC — **desenho pronto, implementação pendente** (backend-senior). Ver detalhes acima antes de retomar — não redesenhar do zero.
